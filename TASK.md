...
E�imin 6 gün sonra "Deutsch-Test für den Beruf B2" sınavı var. Telefonda (iPhone, Safari → Ana Ekrana Ekle) kullanacağı bir çalışma uygulaması yap. **Hız öncelikli: önce çalışan en basit sürüm, sonra iyileştirme.**
...
- GitHub Pages'te çalışsın. Tüm yollar **relative** olsun (`./b2-data.json`), çünkü site bir alt klasörde yayınlanıyor.
...
## PWA
- `manifest.json`: `name: "B2 Prüfungstrainer"`, `short_name: "B2 Beruf"`, `display: "standalone"`, `start_url: "./"`, `scope: "./"`, tema rengi, 192/512 ikon (basit SVG'den PNG ya da tek renk harf ikon yeterli).
...
## Konum
<!-- Seçtiğin seçeneği bırak, diğerini sil -->
- **Seçenek A:** Bu repo (`b2-trainer`) kökünde çalış. GitHub Pages: main branch, root.
- **Seçenek B:** Mevcut `wortkasten` reposunda `pruefung/` klasörü oluştur; tüm dosyalar orada. Wortkasten'in mevcut dosyalarına, manifest'ine ve service worker'ına **dokunma**; bu uygulamanın SW scope'u sadece `./` (pruefung/) olsun. Wortkasten ana sayfasına sadece tek bir "B2 Prüfung →" linki ekle.
